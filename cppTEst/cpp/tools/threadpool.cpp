...
struct threadpool* threadpool_init(int thread_num, int queue_max_num)
{
    ...
    do
    {
        ...
        if (NULL == pool)
        {
            LogE("failed to malloc threadpool!\n");
            break;
        }
        ...
        {
            LogE("failed to init queue_empty!\n");
            break;
        }
        if (pthread_cond_init(&(pool->queue_not_empty), NULL))
        {
            LogE("failed to init queue_not_empty!\n");
            break;
        }
        if (pthread_cond_init(&(pool->queue_not_full), NULL))
        {
            LogE("failed to init queue_not_full!\n");
            break;
        }
        pool->pthreads = (pthread_t *)malloc(sizeof(pthread_t) * thread_num);
        if (NULL == pool->pthreads)
        {
            LogE("failed to malloc pthreads!\n");
            break;
        }
        pool->queue_close = 0;
        pool->pool_close = 0;
...
{
    if(pool == NULL || callback_function == NULL || arg == NULL)
    {
        return -2;
    }
...
int threadpool_add_thread(struct threadpool* pool, int add_num)
{
    pthread_t *free_threads,*new_thread_arrays = NULL;
	int i;
	...
	new_thread_arrays = (pthread_t *)malloc(sizeof(pthread_t) * (pool->thread_num+add_num));
    if (NULL == pool->pthreads)
    {
        LogE("failed to malloc pthreads!\n");
		pthread_mutex_unlock(&(pool->mutex));
	...
	for (i = 0; i < (add_num+pool->thread_num); ++i)
    {
        if(i < pool->thread_num)
			new_thread_arrays[i] = pool->pthreads[i];
		else
            pthread_create(&new_thread_arrays[i], NULL, threadpool_function, (void *)pool);
    }
	free_threads = &pool->pthreads[0];
	pool->pthreads = new_thread_arrays;
	free((void *)free_threads);
	pthread_mutex_unlock(&(pool->mutex));
