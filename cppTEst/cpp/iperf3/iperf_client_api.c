...
iperf_connect(struct iperf_test *test)
{
    ...
    if (test->ctrl_sck < 0) {
        ...
        if (test->lte == 1 || test->lte == 2)
            test->ctrl_sck = netDailLte(test->settings->domain, Ptcp, test->bind_address, 0,
                                        test->server_hostname, test->server_port, test->lte);
        else
...
iperf_run_client(struct iperf_test * test)
{
    ...
    if (iperf_setaffinity(test, test->affinity) != 0){
        LogE_Iperf("iperf debug: fail to iperf_setaffinity");
        return -1;
        }
    ...
    if (iperf_connect(test) < 0){//link fail
        LogE_Iperf("iperf debug: fail to iperf_connect");
        setIperfTestResultToJava(0, 0, 0, 0, 0, ACCESS_DENIED);
        return -1;
        }
    ...
    if (result < 0 && errno != EINTR) {
        LogE_Iperf("iperf debug: fail to select, ret=%d", result);
        i_errno = IESELECT;
        return -1;
    }
    if (result > 0) {
        if (FD_ISSET(test->ctrl_sck, &read_set)) {
            ...
            LogE_Iperf("iperf debug: fail to iperf_handle_message_client, ret=%d", result);
            setIperfTestResultToJava(0, 0, 0, 0, 0, SERVER_ERROR);
            return -1;
        }
    ...
    if (test->state == TEST_RUNNING) {
        ...
        if (iperf_recv(test, &read_set) < 0){
            LogE_Iperf("iperf debug: fail to iperf_recv ");
            return -1;
            }
        } else {
        // Regular mode. Client sends.
        if (iperf_send(test, &write_set) < 0){
            LogE_Iperf("iperf debug: fail to iperf_send ");
            return -1;
            }
        }
        ...
        if (iperf_set_send_state(test, TEST_END) != 0){
            LogE_Iperf("iperf debug: fail to iperf_set_send_state ");
            return -1;
            }
        }
    }
    ...
    // from the client side.
        ...
        if (iperf_recv(test, &read_set) < 0){
        LogE_Iperf("iperf debug: fail to iperf_recv at test End, test->reverse=%d ", test->reverse);
        return -1;
            }
    }
    }
