...
pthread_mutex_t upLock = PTHREAD_COND_INITIALIZER;
...
extern struct iperf_test *ptTest;
extern struct iperf_test *ptTest1;

int globalId = 0;
iperf_size_t lteRecv = 0;
iperf_size_t wifiRecv = 0;
iperf_size_t lteband = 0;
iperf_size_t wifiband = 0;

void clearIperfUploadValue(){
    lteRecv = 0;
    wifiRecv = 0;
    lteband = 0;
    wifiband = 0;
...
iperf_print_intermediate(struct iperf_test *test)
{
    ...
    if (test->num_streams > 1 || test->json_output)
    {
        ...
        if (sp)
        {
            ...
            LogD("iperf3 debug: callBack summary bytes and bps");
            if(test == ptTest){
                lteRecv = bytes;
                lteband = bandwidth;
            }
            else if (test == ptTest1){
                wifiRecv = bytes;
                wifiband = bandwidth;
            }
            
            LogD_Iperf("local isSetLteAddr:%d isSetWifiAddr:%d gMPTCP_config.is_mptcp_init:%d",
            isSetLteAddr,isSetWifiAddr,gMPTCP_config.is_mptcp_init);
            //pthread_mutex_lock(&upLock);
            

            if(test == ptTest) {
                bytes = lteRecv + wifiRecv;
                bandwidth = lteband + wifiband;
                LogD_Iperf("1send len:%llu", (unsigned long long)bytes);
                LogD_Iperf("dBytes:%llu dBps:%f startTime:%f endTime:%f", (unsigned long long)bytes, bandwidth, start_time, end_time);
                setIperfTestResultToJava(sp->socket, (int) start_time, (int) end_time,
                                         (double) (bytes), bandwidth, test->state);
            }
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
    ...
    LogD("iperf3 debug: c call java callBack");
    if(1 == test->num_streams)
    {
        //setIperfTestResultToJava(sp->socket, (int)st, (int)et,  (double) (irp->bytes_transferred), bandwidth, test->state);
        iperf_size_t bytes = irp->bytes_transferred;
        LogD("iperf3 debug: callBack summary bytes and bps when one strem");
        if(test == ptTest){
            lteRecv = bytes;
            lteband = bandwidth;
        }
        ...
        if(isSetLteAddr && isSetWifiAddr && gMPTCP_config.is_mptcp_init ){
            if(test == ptTest) {
                ...
                bandwidth = lteband + wifiband;
                LogD_Iperf("1send len:%llu", (unsigned long long)bytes);
                LogD_Iperf("dBytes:%llu dBps:%f startTime:%f endTime:%f", (unsigned long long)bytes, bandwidth, st, et);
                setIperfTestResultToJava(sp->socket, (int) st, (int) et,
                                         (double) (bytes), bandwidth, test->state);
            }
        }else if(isSetWifiAddr){
            if(test->lte == 0) {
                LogD_Iperf("2send len:%llu", (unsigned long long)bytes);
                setIperfTestResultToJava(sp->socket, (int) st, (int) et,
                                         (double) (bytes), bandwidth, test->state);
            }
        }else if(isSetLteAddr){
            LogD_Iperf("3send len:%llu", (unsigned long long)bytes);
            setIperfTestResultToJava(sp->socket, (int) st, (int) et,
                                     (double) (bytes), bandwidth, test->state);
        }
    }
