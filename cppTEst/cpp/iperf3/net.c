...
#define LOG_MODULE_CURRENT  E_LOG_MODULE_IPERF
...
int connect_noblock(int s, const struct sockaddr *pstSockName, socklen_t u32Namelen)
{
    int flag;
    flag = fcntl(s, F_GETFL, 0);
    fcntl(s, F_SETFL, flag | O_NONBLOCK );

    int ret = connect(s, (struct sockaddr*)pstSockName, u32Namelen);
    if(ret != 0)
    {
        ...
        else // it is connecting
        {
            struct timeval tm;
            tm.tv_sec = 1; //wait 1 second
            tm.tv_usec = 0;
            ...
            FD_SET(s,&wset);
            LogD("selcet start\n");
            int res = select(s+1, NULL, &wset, NULL, &tm);
            LogD("select end\n");
            if(res <= 0)
            {
                LogE("res <= 0\n");
            ...
            if(FD_ISSET(s,&wset))
            {
                ...
                LogD("connetct success\n");
            }
        }
    }

    fcntl(s, F_SETFL, flag);//restore old option
    return 0;
}

int  netDailLte(int domain, int proto, char *local, int local_port, char *server, int port,int lte){
    ...
    if(lte == 1)
        bindNetworkToHandlePacketThoughtJava(1,0);
    else
        bindNetworkToHandlePacketThoughtJava(0,1);
    ...
    if(gMPTCP_config.is_mptcp_init){
        if(protectFd(NULL,s) == -1){
            LogE("protect fd:%d error",s);
            return -1;
        }
    }
    ...
    if (local) {
        ...
        if (bind(s, (struct sockaddr *) local_res->ai_addr, local_res->ai_addrlen) < 0) {
            LogE("%s:%d exit",__func__,__LINE__);
            close(s);
            freeaddrinfo(local_res);
            freeaddrinfo(server_res);
            return -1;
        ...
        freeaddrinfo(local_res);
        LogD("%s:%d bind loacl Ip suceesss",__func__,__LINE__);
    }
...
#endif
    if(connect_noblock(s, (struct sockaddr *) server_res->ai_addr, server_res->ai_addrlen) < 0){
        LogE("%s:%d exit",__func__,__LINE__);
        close(s);
        freeaddrinfo(server_res);
        return -1;
    ...
    LogD("%s:%d connect_noblock suceesss",__func__,__LINE__);
...
netdial(int domain, int proto, char *local, int local_port, char *server, int port)
{
    ...
    if (s < 0) {
        if (local)
            freeaddrinfo(local_res);
        freeaddrinfo(server_res);
        LogE_Iperf("socket error:%s",strerror(errno));
        return -1;
    }
    if(gMPTCP_config.is_mptcp_init){
        if(protectFd(NULL,s) == -1){
            LogE_Iperf("protect fd:%d error",s);
            return -1;
        }
    }
    if (local) {
...
#else
    if(connect_noblock(s, (struct sockaddr *) server_res->ai_addr, server_res->ai_addrlen) < 0){
        LogE_Iperf("%s:%d exit",__func__,__LINE__);
        close(s);
        freeaddrinfo(server_res);
        return -1;
    }
